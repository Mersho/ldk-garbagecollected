...
public class OutPoint : CommonBase {
	...
	public void set_txid(byte[] val) {
		Bindings.OutPointSetTxid(this.ptr, InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(val, 32)));
		GC.KeepAlive(this);
		GC.KeepAlive(val);
	}
	...
	public static OutPoint of(byte[] txid_arg, short index_arg) {
		long ret = Bindings.OutPointNew(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(txid_arg, 32)), index_arg);
