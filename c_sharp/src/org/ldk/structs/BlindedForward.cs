...
public class BlindedForward : CommonBase {
	...
	public void set_inbound_blinding_point(byte[] val) {
		Bindings.BlindedForwardSetInboundBlindingPoint(this.ptr, InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(val, 33)));
		GC.KeepAlive(this);
		GC.KeepAlive(val);
	}
	...
	public static BlindedForward of(byte[] inbound_blinding_point_arg, BlindedFailure failure_arg) {
		long ret = Bindings.BlindedForwardNew(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(inbound_blinding_point_arg, 33)), failure_arg);
