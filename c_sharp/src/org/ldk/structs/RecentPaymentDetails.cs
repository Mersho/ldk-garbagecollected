...
public class RecentPaymentDetails : CommonBase {
	...
	public static RecentPaymentDetails awaiting_invoice(byte[] payment_id) {
		long ret = Bindings.RecentPaymentDetailsAwaitingInvoice(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_id, 32)));
	...
	public static RecentPaymentDetails pending(byte[] payment_id, byte[] payment_hash, long total_msat) {
		long ret = Bindings.RecentPaymentDetailsPending(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_id, 32)), InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_hash, 32)), total_msat);
		GC.KeepAlive(payment_id);
	...
	public static RecentPaymentDetails fulfilled(byte[] payment_id, org.ldk.structs.Option_ThirtyTwoBytesZ payment_hash) {
		long ret = Bindings.RecentPaymentDetailsFulfilled(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_id, 32)), payment_hash.ptr);
		GC.KeepAlive(payment_id);
		GC.KeepAlive(payment_hash);
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.RecentPaymentDetails ret_hu_conv = org.ldk.structs.RecentPaymentDetails.constr_from_ptr(ret);
	...
	public static RecentPaymentDetails abandoned(byte[] payment_id, byte[] payment_hash) {
		long ret = Bindings.RecentPaymentDetailsAbandoned(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_id, 32)), InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(payment_hash, 32)));
		GC.KeepAlive(payment_id);
		GC.KeepAlive(payment_hash);
		if (ret >= 0 && ret <= 4096) { return null; }
		org.ldk.structs.RecentPaymentDetails ret_hu_conv = org.ldk.structs.RecentPaymentDetails.constr_from_ptr(ret);
