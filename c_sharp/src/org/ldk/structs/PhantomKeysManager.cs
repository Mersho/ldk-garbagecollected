...
public class PhantomKeysManager : CommonBase {
	...
	public static PhantomKeysManager of(byte[] seed, long starting_time_secs, int starting_time_nanos, byte[] cross_node_seed) {
		long ret = Bindings.PhantomKeysManagerNew(InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(seed, 32)), starting_time_secs, starting_time_nanos, InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(cross_node_seed, 32)));
		GC.KeepAlive(seed);
	...
	public InMemorySigner derive_channel_keys(long channel_value_satoshis, byte[] _params) {
		long ret = Bindings.PhantomKeysManagerDeriveChannelKeys(this.ptr, channel_value_satoshis, InternalUtils.encodeUint8Array(InternalUtils.check_arr_len(_params, 32)));
		GC.KeepAlive(this);
